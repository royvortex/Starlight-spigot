...
public class UserManager extends AbstractUserManager<StarlightPlugin, SunUser> {

    private final DataHandler dataHandler;

    public UserManager(@NonNull StarlightPlugin plugin, @NonNull DataHandler dataHandler) {
    ...
    public CompletableFuture<InetAddress> loadInetAddress(@NonNull UUID playerId) {
        ...
        if (target != null) return CompletableFuture.completedFuture(STUtils.getInetAddress(target).orElse(null));
