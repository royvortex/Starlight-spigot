...
import me.royvortex.starlight.STConfigTypes;
...
public class ModuleLoader {
    ...
    public ModuleLoader(@NotNull StarlightPlugin plugin, @NotNull ModuleRegistry moduleRegistry) {
        this.plugin = plugin;
    ...
    public void loadAll() {
        FileConfig config = FileConfig.load(this.plugin.getDataFolder().getAbsolutePath(), STFiles.FILE_MODULES);
        ...
        this.definitionMap.forEach((id, defaultDefinition) -> {
            ...
            ModuleDefinition definition = config.get(STConfigTypes.MODULE_DEFINITION, "Modules." + id, defaultDefinition);
    ...
    private boolean loadModule(@NotNull String id, @NotNull ModuleDefinition definition) throws ModuleLoadException {
        ...
        Path path = Paths.get(this.plugin.getDataFolder() + STFiles.DIR_MODULES, id);
