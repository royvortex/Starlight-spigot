...
public class EnchantCommandsProvider extends AbstractCommandProvider {
    ...
    public EnchantCommandsProvider(@NotNull StarlightPlugin plugin, @NotNull EssentialModule module, @NotNull UserManager userManager) {
        super(plugin);
        this.module = module;
        ...
        return this.loadPlayerOrSenderAndRunInMainThread(context, arguments, this.module, this.userManager, target -> {
            ...
            if (target != context.getSender()) {
                this.module.sendPrefixed(MESSAGE_ENCHANTED_FEEDBACK, context.getSender(), replacer -> replacer
                    .with(CommonPlaceholders.PLAYER.resolver(target))
                    .with(STPlaceholders.GENERIC_TYPE, () -> Lang.EQUIPMENT_SLOT.getLocalized(slot))
                    .with(STPlaceholders.GENERIC_ITEM, () -> ItemUtil.getNameSerialized(item))
                    .with(STPlaceholders.GENERIC_NAME, () -> LangUtil.getSerializedName(enchant))
                    .with(STPlaceholders.GENERIC_AMOUNT, () -> LangUtil.getEnchantmentLevelLang(level))
                );
            }
