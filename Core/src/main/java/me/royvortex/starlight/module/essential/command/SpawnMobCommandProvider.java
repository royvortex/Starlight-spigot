...
import static me.royvortex.starlight.STPlaceholders.GENERIC_AMOUNT;
...
public class SpawnMobCommandProvider extends AbstractCommandProvider {
    ...
    public SpawnMobCommandProvider(@NotNull StarlightPlugin plugin) {
        super(plugin);
    }
    ...
    private boolean spawnMob(@NotNull CommandContext context, @NotNull ParsedArguments arguments) {
        ...
        context.send(MESSAGE_SPAWNED_FEEDBACK, replacer -> replacer
            .replace(STPlaceholders.GENERIC_AMOUNT, String.valueOf(amount))
            .replace(STPlaceholders.GENERIC_TYPE, LangUtil.getSerializedName(entityType))
        );
        return true;
    }
}
