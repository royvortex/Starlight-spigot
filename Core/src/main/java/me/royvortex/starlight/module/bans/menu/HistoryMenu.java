package me.royvortex.starlight.module.bans.menu;
...
import static me.royvortex.starlight.module.bans.BansPlaceholders.*;
...
public class HistoryMenu extends AbstractObjectMenu<HistoryMenu.Data> implements LangContainer {
    ...
    private static final IconLocale LOCALE_ACTIVE = LangEntry.iconBuilder("Bans.UI.History.Icon.Active")
        .rawName(WHITE.wrap(STPlaceholders.GENERIC_TYPE) + DARK_GRAY.wrap(" • " + GREEN.wrap("Active")))
        .rawLore(
    ...
    private static final IconLocale LOCALE_PAUSED = LangEntry.iconBuilder("Bans.UI.History.Icon.Paused")
        .rawName(WHITE.wrap(STPlaceholders.GENERIC_TYPE) + DARK_GRAY.wrap(" • " + YELLOW.wrap("Inactive")))
        .rawLore(
            ITALIC.and(DARK_GRAY).wrap("\"" + PUNISHMENT_REASON + "\""),
            "",
            RED.wrap("➥ " + GRAY.wrap("Staff: ") + PUNISHMENT_WHO),
    ...
    private static final IconLocale LOCALE_EXPIRED = LangEntry.iconBuilder("Bans.UI.History.Icon.Expired")
        .rawName(WHITE.wrap(STPlaceholders.GENERIC_TYPE) + DARK_GRAY.wrap(" • " + GREEN.wrap("Expired")))
        .rawLore(
            ITALIC.and(DARK_GRAY).wrap("\"" + PUNISHMENT_REASON + "\""),
            "",
            RED.wrap("➥ " + GRAY.wrap("Staff: ") + PUNISHMENT_WHO),
    ...
    public HistoryMenu(@NotNull StarlightPlugin plugin, @NotNull BansModule module) {
        super(MenuType.GENERIC_9X5, BLACK.wrap("[%s] History for %s".formatted(STPlaceholders.GENERIC_TYPE, STPlaceholders.GENERIC_TARGET)), Data.class);
        this.plugin = plugin;
        this.module = module;
    ...
    protected String getRawTitle(@NotNull ViewerContext context) {
        ...
        return PlaceholderContext.builder()
            .with(STPlaceholders.GENERIC_TYPE, () -> TYPE_LOCALE.getLocalized(data.type))
            .with(STPlaceholders.GENERIC_TARGET, data.userInfo::name)
            .build()
        ...
        this.addDefaultButton("sort_mode", MenuItem.builder()
            .defaultState(ItemState.defaultBuilder()
                .icon(NightItem.fromType(Material.COMPARATOR)
                    ...
                    .setLore(List.of(
                        DARK_GRAY.wrap("»" + GRAY.wrap(" Selected: ") + WHITE.wrap(STPlaceholders.GENERIC_MODE)),
                        "",
                ...
                .displayModifier((context, item) -> {
                    item.replace(builder -> builder.with(STPlaceholders.GENERIC_MODE, () -> BansLang.SORT_MODE.getLocalized(this.getObject(context).sortMode)));
                })
                .action(context -> {
                    Data data = this.getObject(context);
        ...
        this.addDefaultButton("show_expired", MenuItem.builder()
            .defaultState(ItemState.defaultBuilder()
                .icon(NightItem.fromType(Material.CLOCK)
                    ...
                    .setLore(List.of(
                        DARK_GRAY.wrap("»" + GRAY.wrap(" Status: ") + WHITE.wrap(STPlaceholders.GENERIC_STATE)),
                        "",
                ...
                .displayModifier((context, item) -> {
                    item.replace(builder -> builder.with(STPlaceholders.GENERIC_STATE, () -> CoreLang.STATE_ENABLED_DISALBED.get(this.getObject(context).showExpired)));
                })
                .action(context -> {
                    Data data = this.getObject(context);
            ...
            .itemProvider((context, punishment) -> {
                ...
                return icon.copy()
                    ...
                    .replace(builder -> builder
                        .with(STPlaceholders.GENERIC_TYPE, () -> BansLang.PUNISHMENT_TYPE.getLocalized(data.type))
