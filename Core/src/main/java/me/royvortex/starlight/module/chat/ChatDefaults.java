...
import static me.royvortex.starlight.module.chat.ChatPlaceholders.*;
...
public class ChatDefaults {
    ...
    public static final String DEFAULT_CHANNEL_FORMAT = GRAY.wrap("[" + CHANNEL_NAME + "]") + " " + STPlaceholders.GENERIC_FORMAT;
    public static final String DEFAULT_LOCAL_CHANNEL_ID = "local";

    public static final String DEFAULT_USER_FORMAT    = CommonPlaceholders.PLAYER_NAME + ": " + STPlaceholders.GENERIC_FORMAT;
    ...
    public static Map<String, FormatDefinition> getDefaultFormats() {
        ...
        map.put(DEFAULT, new FormatDefinition(
            0,
            "%rank_member%%player_info%: " + GRAY.wrap(STPlaceholders.GENERIC_MESSAGE),
            Set.of(STPlaceholders.WILDCARD)
        ));

        map.put("owner", new FormatDefinition(
            100,
            "%rank_owner%%player_info%: " + GRAY.wrap(STPlaceholders.GENERIC_MESSAGE),
            Set.of("admin", "owner")
        ));
    ...
    public static Map<String, GroupMention> getDefaultMentions() {
        ...
        map.put("all", new GroupMention(SOFT_AQUA.wrap("@All"), Set.of(STPlaceholders.WILDCARD)));
    ...
    public static Map<SpyType, String> getDefaultSpyFormat() {
        ...
        map.put(SpyType.SOCIAL, SOFT_RED.wrap("[SocialSpy]") + " " + GRAY.wrap(PLAYER_NAME + " ➡ " + STPlaceholders.GENERIC_TARGET + ":") + " " + GRAY.wrap(STPlaceholders.GENERIC_MESSAGE));
        map.put(SpyType.COMMAND, SOFT_RED.wrap("[CommandSpy]") + " " + GRAY.wrap(PLAYER_NAME + " executed a command:") + " " + GRAY.wrap(STPlaceholders.GENERIC_MESSAGE));
        map.put(SpyType.CHAT, SOFT_RED.wrap("[ChatSpy]") + " " + GRAY.wrap("[" + CHANNEL_NAME + "]" + " " + PLAYER_NAME + ":") + " " + GRAY.wrap(STPlaceholders.GENERIC_MESSAGE));
        return map;
    }
}
