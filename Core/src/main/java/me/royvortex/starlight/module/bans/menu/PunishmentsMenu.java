...
public class PunishmentsMenu extends AbstractObjectMenu<PunishmentsMenu.Data> implements LangContainer {
    ...
    public PunishmentsMenu(@NotNull StarlightPlugin plugin, @NotNull BansModule module) {
        super(MenuType.GENERIC_9X5, BLACK.wrap("[%s] List".formatted(STPlaceholders.GENERIC_TYPE)), Data.class);
        this.plugin = plugin;
        this.module = module;
    ...
    public void defineDefaultLayout() {
        ...
        this.addDefaultButton("show_expired", MenuItem.builder()
            .defaultState(ItemState.defaultBuilder()
                ...
                .displayModifier((context, item) -> {
                    item.replace(builder -> builder.with(STPlaceholders.GENERIC_STATE, () -> su.nightexpress.nightcore.core.config.CoreLang.STATE_ENABLED_DISALBED.get(this.getObject(context).showExpired)));
                })
                .action(context -> {
                    Data data = this.getObject(context);
