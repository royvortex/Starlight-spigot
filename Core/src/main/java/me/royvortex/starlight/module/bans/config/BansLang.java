package me.royvortex.starlight.module.bans.config;
...
import me.royvortex.starlight.module.bans.menu.SortMode;
...
public class BansLang implements LangContainer {
    ...
    public static final TextLocale ALTS_GLOBAL_ENTRY = LangEntry.builder("Bans.Alts.Global.Entry").text(
        DARK_GRAY.wrap("» ") + RUN_COMMAND.with("/" + STSharedConsts.COMMAND_PLAYER_INFO + " " + GENERIC_NAME).wrap(SHOW_TEXT.with(GRAY.wrap("Click for more details.")).wrap(RED.wrap(GENERIC_NAME)))
    );
