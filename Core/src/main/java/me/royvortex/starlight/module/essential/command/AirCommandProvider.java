...
import me.royvortex.starlight.command.mode.ModifyMode;
...
public class AirCommandProvider extends AbstractCommandProvider {
    ...
    public AirCommandProvider(@NotNull StarlightPlugin plugin, @NotNull EssentialModule module, @NotNull UserManager userManager) {
        super(plugin);
        this.module = module;
        ...
        return this.loadPlayerOrSenderAndRunInMainThread(context, arguments, this.module, this.userManager, target -> {
            ...
            if (target != context.getSender()) {
                this.module.sendPrefixed(message, context.getSender(), replacer -> replacer
                    .with(STPlaceholders.GENERIC_CURRENT, () -> String.valueOf(target.getRemainingAir()))
                    .with(STPlaceholders.GENERIC_MAX, () -> String.valueOf(target.getMaximumAir()))
                    .with(STPlaceholders.GENERIC_AMOUNT, () -> NumberUtil.format(amount))
