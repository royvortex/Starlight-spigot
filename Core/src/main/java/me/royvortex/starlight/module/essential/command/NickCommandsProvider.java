...
import static me.royvortex.starlight.STPlaceholders.*;
...
public class NickCommandsProvider extends AbstractCommandProvider {
    ...
    public NickCommandsProvider(@NotNull StarlightPlugin plugin, @NotNull EssentialModule module, @NotNull EssentialSettings settings, @NotNull UserManager userManager) {
        super(plugin);
        this.module = module;
    ...
    private boolean changeNick(@NotNull CommandContext context, @NotNull ParsedArguments arguments) {
        ...
        if (!player.hasPermission(NICK_BYPASS_LENGTH)) {
            if (raw.length() < this.settings.nickMinLength.get()) {
                this.module.sendPrefixed(COMMAND_NICK_CHANGE_ERROR_TOO_SHORT, context.getSender(), replacer -> replacer.with(STPlaceholders.GENERIC_AMOUNT, () -> String.valueOf(this.settings.nickMinLength.get())));
                return false;
            }
            if (raw.length() > this.settings.nickMaxLength.get()) {
                this.module.sendPrefixed(COMMAND_NICK_CHANGE_ERROR_TOO_LONG, context.getSender(), replacer -> replacer.with(STPlaceholders.GENERIC_AMOUNT, () -> String.valueOf(this.settings.nickMaxLength.get())));
                return false;
            }
        }
        ...
        this.module.sendPrefixed(COMMAND_NICK_CHANGE_DONE, context.getSender(), replacer -> replacer.with(STPlaceholders.GENERIC_NAME, () -> finalNick));
        ...
        return this.loadPlayerWithDataAndRunInMainThread(context, arguments, this.module, this.userManager, (user, target) -> {
            ...
            if (context.getSender() != user.player().orElse(null)) {
                ...
                this.module.sendPrefixed(COMMAND_NICK_SET_TARGET, context.getSender(), replacer -> replacer
                    .with(STPlaceholders.GENERIC_NAME, () -> finalNick1)
                    .with(STPlaceholders.PLAYER_NAME, user::getName)
                );
            }

            if (!context.hasFlag(CommandArguments.FLAG_SILENT)) {
                String finalNick = nick;
                this.module.sendPrefixed(COMMAND_NICK_SET_NOTIFY, target, replacer -> replacer.with(STPlaceholders.GENERIC_NAME, () -> finalNick));
            }
        });
    }
}
