...
import me.royvortex.starlight.api.event.PlayerTeleportRequestEvent;
...
import me.royvortex.starlight.module.ptp.command.PTPCommands;
import me.royvortex.starlight.module.ptp.config.PTPSettings;
import me.royvortex.starlight.module.ptp.config.PTPLang;
import me.royvortex.starlight.module.ptp.config.PTPPerms;
import me.royvortex.starlight.module.ptp.listener.PTPListener;
import me.royvortex.starlight.module.ptp.request.TeleportMode;
import me.royvortex.starlight.module.ptp.request.TeleportRequest;
    ...
    public boolean sendRequest(@NotNull Player sender, @NotNull Player target, @NotNull TeleportMode mode) {
        ...
        if (requestHas != null && !requestHas.isExpired()) {
            this.sendPrefixed(PTPLang.REQUEST_SEND_ERROR_COOLDOWN, sender, builder -> builder
                .with(STPlaceholders.GENERIC_TIME, () -> TimeFormats.formatDuration(requestHas.getExpireDate(), TimeFormatType.LITERAL))
            );
            return false;
        }
