...
public class TeleportManager extends SimpleManager<StarlightPlugin> {

    private final SunNMS internals;

    public TeleportManager(@NonNull StarlightPlugin plugin, @Nullable SunNMS internals) {
        super(plugin);
    ...
    public boolean teleport(@NonNull TeleportContext context, @NonNull TeleportType type) {
        StarlightPlayerTeleportEvent event = new StarlightPlayerTeleportEvent(context, type);
