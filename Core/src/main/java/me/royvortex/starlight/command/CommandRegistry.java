package me.royvortex.starlight.command;
...
import me.royvortex.starlight.STFiles;
import me.royvortex.starlight.STPlaceholders;
import me.royvortex.starlight.StarlightPlugin;
import me.royvortex.starlight.command.provider.CommandProvider;
import me.royvortex.starlight.command.provider.definition.HubDefinition;
import me.royvortex.starlight.command.provider.definition.LiteralDefinition;
...
public class CommandRegistry extends SimpleManager<StarlightPlugin> {
    ...
    public CommandRegistry(@NotNull StarlightPlugin plugin) {
        super(plugin);
    ...
    private void registerCommands() {
        this.providers.forEach((providerId, provider) -> {
            FileConfig config = FileConfig.load(this.plugin.getDataFolder() + STFiles.DIR_COMMANDS, FileConfig.withExtension(providerId));
    ...
    private void register(@NotNull NightCommand command) {
        ...
        if (!command.register()) {
            this.plugin.warn("Command '%s' was not registered with the passed in label, which indicates the Starlight's fallback prefix was used one or more time. This usually means that there is a vanilla command with the same label.");
        }
            ...
            if (cooldown != 0 && user != null && !player.hasPermission(Perms.BYPASS_COMMAND_COOLDOWN)) {
                ...
                if (expireDate != null && !TimeUtil.isPassed(expireDate)) {
                    (expireDate < 0 ? Lang.COMMAND_COOLDOWN_ONE_TIME : Lang.COMMAND_COOLDOWN_DEFAULT).message().sendWith(player, replacer -> replacer
                        .with(STPlaceholders.GENERIC_TIME, () -> TimeFormats.formatDuration(expireDate, TimeFormatType.LITERAL))
                        .with(STPlaceholders.GENERIC_COMMAND, () -> "/" + context.getInput())
                    );
                    return false;
                }
            }
    ...
    private void unregisterConflicts(@NotNull NightCommand command) {
        ...
        aliases.forEach(alias -> {
            CommandUtil.getCommand(alias).ifPresent(other -> {
                ...
                if (result) {
                    this.plugin.info("Unregistered conflicting '%s' (%s) command in favor of Starlight's alternative.".formatted(other.getName(), owner));
                }
                else {
                    this.plugin.warn("Could not unregister conflicting command '%s' (%s) in favor of Starlight's alternative.".formatted(other.getName(), owner));
                }
            });
        });
    }
